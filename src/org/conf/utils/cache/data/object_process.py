...
def write_to_csg(obj_id, data_points, surface_color1,
    ...
    cell_array : list
        ...
        sNp0, sNp1, ..., sNpKN]
    """
    with open(png_dir + str(obj_id) + ".csg", "w") as f:
        f.write("group() {\n")
        for i in range(len(cell_array)):
            ...
            f.write(", faces = ")
            len_ca = len(cell_array[i])
            faces_array = [np.arange(len_ca).tolist()]
            """
            # This would be preferable but it needs more work.
            for j in range(0, len_ca - 1, 2):
                faces_array.append([(j + 0) % len_ca, (j + 1) % len_ca, (j + 2) % len_ca])
            """
...
def csgtoob3(start_file, max_files):
    ...
    """
    for i in range(start_file, start_file + max_files):
        if (os.path.isfile(png_dir + str(i) + ".csg")):
            print("Processing file %d of %d (%.1f%%)"
                  % (i, max_files, 100*(i-start_file)/max_files))
            with open(png_dir + str(i) + ".csg", "r") as f:
                data_read = f.read()
...
def plotob3(start_file, max_files):
    for i in range(start_file, start_file + max_files):
        if (os.path.isfile(ob3_dir + str(i))):
            ...
            n_sides = data[1]
            data_points = data[2]
            points_per_cell = data[3]
            surface_color1 = data[4]
            surface_color2 = data[5]
            some_array = data[6]
            cell_array = data[7]
            x = data_points[0]
            y = data_points[2]
            ...
            for j in range(len(vertices)):
                ...
                if (surface_color1[j] != 0x7fff):
                    tri.set_color(rgb_to_hex(surface_color1[j]))
                else:
                    tri.set_color(rgb_to_hex(surface_color2[j]))
            ...
            if max_val > 1000:
                max_val = 1000
            max_val *= (1.1 if max_val > 0 else 0.9)
            min_val = min([min(x), min(y), min(z)])
            if min_val < -1000:
                min_val = -1000
            min_val *= (1.1 if min_val < 0 else 0.9)
            ax.set_xlim3d(min_val, max_val)
            ax.set_ylim3d(min_val, max_val)
            ax.set_zlim3d(min_val, max_val)
            ax.set_aspect("equal")
            ax.view_init(elev=210, azim=60)
            plt.savefig(png_dir + str(i) + ".png")
            # plt.show()
            plt.close(fig)
