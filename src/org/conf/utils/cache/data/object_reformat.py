...
if __name__ == "__main__":
    ...
    for i in range(0, max_files, 1):
        if (os.path.isfile(src + "object" + str(i) + ".ob3")):
            print("Processing file %d of %d (%.1f%%)"
                  % (i, max_files, 100*i/max_files))
            with open(src + "object" + str(i) + ".ob3", "rb") as s:
                ...
                if (len(data_read) != file_size_read):
                    file_size = sp.array([len(data_read) + 4], dtype=sp.int32)
            ...
            print("%sobject%s.ob3 not found" % (dst, i))
