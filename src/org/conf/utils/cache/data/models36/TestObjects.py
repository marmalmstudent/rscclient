...
def rgba_to_hex(val):
    if val < 0:
        argb = np.array([-(val+1)], dtype=np.int16)
        r = ((argb >> 10) & 0x1f) << 3
        g = ((argb >> 5) & 0x1f) << 3
        b = (argb & 0x1f) << 3
    ...
    if (os.path.isfile("object"+str(i)+".ob3")):
        ...
        with open("object"+str(i)+".csg", "w") as f:
            ...
            for j in range(0, nSides, 1):
                ...
                f.write("        group() {\n                color("
                        + str(rgba_to_float(
                            surfaceColor1[j] if (surfaceColor1[j] != 0x7fff) else surfaceColor2[j])) +
                        ") {\n                        polyhedron(points = ")
                points_write = np.transpose([
                    dataPoints[0][min(tmpArray):max(tmpArray)+1],
                    dataPoints[2][min(tmpArray):max(tmpArray)+1],
                    dataPoints[1][min(tmpArray):max(tmpArray)+1]])
                f.write(str(points_write.tolist()))
                f.write(", faces = ")
                f.write(str([(tmpArray-min(tmpArray)).tolist()]))
                f.write(", convexity = 1);\n                }\n        }\n")
            f.write("}")
